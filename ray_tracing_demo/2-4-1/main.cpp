...
vec2<double>* generate()
{
	vec2<double> *p=new vec2<double>[256];
	for(int i=0;i<256;i++)p[i]=vec2<double>(random_unit_figure()*2-1,random_unit_figure()*2-1).ret_unitization();
	return p;
}
vec2<double>* _randomvalue;
...
const value_type interp(vec2<double>list[2][2],value_type u,value_type v)
{
	...
	for(int i=0;i<=1;i++)
	for(int j=0;j<=1;j++)accumulate+=(i*u+(1-i)*(1-u))*(j*v+(1-j)*(1-v))*dot(list[i][j],vec2<double>(u-i,v-j));//这样算出来最大值只有二分之根号三 sqrt((u-i)*(u-i)+(v-j)*(v-j)+(w-k)*(w-k))
	return accumulate;
}
...
int main()
{
	...
	for(int j=0;j<width;j++)
	{
		...
		else setpoint(&bmpImage,i,j,vec_type(255,255,255)*sqrt(abs(t)));
		// else if(t>0)	setpoint(&bmpImage,i,j,vec_type(0,255,0)*sqrt(t));
		// else setpoint(&bmpImage,i,j,vec_type(0,0,255)*sqrt(-t));
	}
